...
const STYLE_ID = 'podium-skin-3d-css-v5';
function ensureStyles() {
  if (document.getElementById(STYLE_ID)) return;
  /* Remove any old style versions */
  ['podium-skin-3d-css','podium-skin-3d-css-v4'].forEach(id => {
...
.mc-fw-slot--l { left: 4px; }
.mc-fw-slot--r { right: 4px; }
...
.mc-fw-cap-bar  { position:absolute; top:5px; left:0; width:14px; height:4px; background:#5c2e0e; box-shadow:inset 0 1px 0 rgba(255,255,255,0.15),inset 0 -1px 0 rgba(0,0,0,0.3); }
.mc-fw-cap-knob { position:absolute; top:0; left:4px; width:6px; height:7px; background:#6b3417; }

.mc-fw-body {
  width:10px; height:22px;
  background: repeating-linear-gradient(-45deg,#cc1111 0px,#cc1111 3px,#f2f2f2 3px,#f2f2f2 6px);
...
@keyframes mc-exhaust-flicker {
  0%  { height:10px; opacity:1.0; }
  100%{ height:7px;  opacity:0.7; }
}
...
.mc-fw-slot--l .mc-fw-burst { animation:mc-burst-appear 2.5s ease-out infinite; animation-delay:0s; }
.mc-fw-slot--r .mc-fw-burst { animation:mc-burst-appear 2.5s ease-out infinite; animation-delay:1.25s; }
...
/* ══════════════════════════════════════════════════════════════
   ...
   z-index:4 ensures it renders ABOVE the skinview3d canvas
   positioned at foot level, puffs fly left+right behind steps
   ══════════════════════════════════════════════════════════════ */
.dust-cloud-overlay {
  position: absolute;
  bottom: 0px;
...
.dust-puff {
  position: absolute;
  border-radius: 50%;
  background: rgba(225,218,205,0.95);
  border: 2px solid rgba(195,188,172,0.6);
}
/* Left puffs — expand to the left */
.dp-l1 { width:22px; height:22px; left:29px; bottom:0;  animation:dp-left 0.42s ease-out infinite; animation-delay:0.00s; }
.dp-l2 { width:16px; height:16px; left:27px; bottom:3px; animation:dp-left 0.42s ease-out infinite; animation-delay:0.21s; }
/* Right puffs — expand to the right */
.dp-r1 { width:22px; height:22px; left:29px; bottom:0;  animation:dp-right 0.42s ease-out infinite; animation-delay:0.105s; }
.dp-r2 { width:16px; height:16px; left:31px; bottom:3px; animation:dp-right 0.42s ease-out infinite; animation-delay:0.315s; }
/* Center puff — rises up */
.dp-c  { width:18px; height:18px; left:31px; bottom:0;  animation:dp-up    0.42s ease-out infinite; animation-delay:0.05s; }

@keyframes dp-left {
  0%   { transform:scale(0.2) translate(  0px,  0px); opacity:0;    }
  15%  { transform:scale(0.8) translate( -8px, -4px); opacity:1.0;  }
  55%  { transform:scale(1.2) translate(-18px, -8px); opacity:0.65; }
  100% { transform:scale(1.6) translate(-30px,-14px); opacity:0;    }
}
@keyframes dp-right {
  0%   { transform:scale(0.2) translate( 0px,  0px); opacity:0;    }
  15%  { transform:scale(0.8) translate( 8px, -4px); opacity:1.0;  }
...
@keyframes dp-up {
  0%   { transform:scale(0.2) translate(0px,  0px); opacity:0;    }
  15%  { transform:scale(0.7) translate(0px, -6px); opacity:0.85; }
  55%  { transform:scale(1.1) translate(0px,-12px); opacity:0.55; }
  100% { transform:scale(1.4) translate(0px,-20px); opacity:0;    }
}
`;
...
export default function PodiumSkin3D({ username, rank }: Props) {
  ...
  useEffect(() => {
    ...
    import('skinview3d').then((sv3d) => {
      ...
      /* ══════════════════════════════════════════════════════════
         #3  RUNNING — speed 2.8, expressive sprint, dust synced
         ══════════════════════════════════════════════════════════ */
      if (rank === 3) {
        viewer.animation = new sv3d.FunctionAnimation((player: any, progress: number) => {
          try {
            ...
            const t = progress * 2.8;   /* fast sprint */

            s.body.rotation.x = 0.20;
            s.body.rotation.y = 0;
            s.body.rotation.z = Math.sin(t * 2) * 0.04;

            const arm = Math.sin(t) * 1.10;
            ...
            if (!s.head.userData.crownDone) {
              ...
              import('three').then((T: any) => {
                ...
                const bx=(mat:any,w:number,h:number,d:number,x:number,y:number,z:number)=>{
                  const m=new T.Mesh(new T.BoxGeometry(w,h,d),mat); m.position.set(x,y,z); g.add(m);
                };
                const BW=10.0,BH=1.0,BT=0.80,inner=BW-BT*2;
                const FZ=BW/2-BT/2,BKZ=-(BW/2-BT/2),LX=-(BW/2-BT/2),RX=BW/2-BT/2;
                bx(facesWall,BW,BH,BT,0,BH/2,FZ); bx(facesWall,BW,BH,BT,0,BH/2,BKZ);
                bx(facesWall,BT,BH,inner,LX,BH/2,0); bx(facesWall,BT,BH,inner,RX,BH/2,0);
                const MW=0.88,MD=BT,MH=1.45,MHC=2.20,yB=BH;
                [-3.4,-2.26,-1.13,0,1.13,2.26,3.4].forEach((x,i)=>{
                  const h=i===3?MHC:MH, mat=i===3?facesCenter:facesTop;
                  bx(mat,MW,h,MD,x,yB+h/2,FZ); bx(mat,MW,h,MD,x,yB+h/2,BKZ);
                });
                [-2.0,0,2.0].forEach(z=>{
                  bx(facesTop,MD,MH,MW,LX,yB+MH/2,z); bx(facesTop,MD,MH,MW,RX,yB+MH/2,z);
                });
                ...
                ([[-2.8,gemPurple],[-0.9,gemCyan],[0.9,gemBlue],[2.8,gemGreen]] as [number,any][]).forEach(([x,mat])=>{
                  const gem=new T.Mesh(new T.BoxGeometry(GS,GS,GD),mat); gem.position.set(x,GY,GZ); g.add(gem);
                });
                g.position.set(0,6.0,0); s.head.add(g);
              }).catch(()=>{});
            }
          ...
          {(['l','r'] as const).map(side => (
            <div key={side} className={`mc-fw-slot mc-fw-slot--${side}`}>
              ...
              <div className="mc-fw-burst">
                {BURST_ANGLES.map(a => (
                  <div key={a} className="mc-burst-ray" style={{ transform:`rotate(${a}deg) translateY(-9px)` }} />
                ))}
              </div>
            </div>
          ))}
