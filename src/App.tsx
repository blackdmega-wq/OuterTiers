...
function TouchTrail() {
  useEffect(() => {
    ...
    function draw(now: number) { rafId = 0; ctx.clearRect(0, 0, W, H); const cutoff = now - TRAIL_LIFE_MS; while (pts.length > 0 && pts[pts.length - 1].t < cutoff) pts.pop(); if (pts.length > 1) { ctx.save(); ctx.shadowColor = 'rgba(91,164,245,0.85)'; ctx.shadowBlur = 12; for (let i = 0; i < pts.length; i++) { const ageFrac = (now - pts[i].t) / TRAIL_LIFE_MS; const idxFrac = i / (pts.length - 1); const fade = Math.max(0, 1 - ageFrac); const alpha = fade * (1 - idxFrac * 0.55) * 0.92; const radius = Math.max(1.5, 8.5 * (1 - idxFrac * 0.72) * fade); if (alpha < 0.01) continue; const isMid = idxFrac > 0.3 && idxFrac < 0.7; ctx.globalAlpha = alpha; ctx.beginPath(); ctx.arc(pts[i].x, pts[i].y, radius, 0, Math.PI * 2); ctx.fillStyle = `rgb(${isMid ? 230 : 180},${isMid ? 195 : 225},255)`; ctx.fill(); } ctx.restore(); } if (isActive || pts.length > 0) { rafId = requestAnimationFrame(draw); } }
